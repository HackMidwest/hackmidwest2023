...
export class PennyArtHomeComponent implements OnInit {
  ...
  onUpload(e: Event) {
    this.messageService.add({severity:'success', summary: 'Success', detail: 'Image Uploaded Succesfully'});
    this.upload = true;
    this.pennyartService.getMyArt().subscribe((resp: any) => {
      this.images = resp;
    })
  ...
  sendGreetings(e: Event) {
    ...
    this.messageService.add({severity:'success', summary: 'Success', detail: 'Greeting Sent Succesfully'});
  }
